...
AIGC：本记录由 Codex 于 2026-09-06 根据代码、构建产物、本机 Chromium 检查和用户确认的取舍整理，2026-09-08 补充已确认的字体修复。这里只记录可验证的行为与资源规模，不把单次本地加载时间视为线上性能结论。楷体选型仍留在独立分支，不在此处恢复。
...
| 字体 | 多套 Google Fonts 和中文字体 CSS 仍在首屏请求路径上 | 2026-09-08 单独恢复中文粗体与 Fira Code 加载修复；楷体选型、斜体实验和相应授权决策继续暂缓 |
...
中文 `strong`、`b` 使用 STIX Two Text、Source Han Serif CN VF、Noto Serif SC、serif 的字体栈，移除提前匹配系统无衬线字体的 `sans-serif`。Google Fonts 请求补上 Fira Code 的 400、700 字重并保留 `display=swap`；既有代码字体声明不变。二者从字体实验中单独移入主分支，`codex/font-exploration` 只保留楷体相关试排和禁止合成斜体的实验。

新增两项回归测试后，36 项 Python 模板与数学处理测试通过。隔离的 headless Chromium 使用构建后的样式和真实字体 CDN 检查，中文粗体实际命中 `SourceHanSerifCNVF-Bold`，英文粗体仍为 `STIXTwoText_Bold`，代码命中下载的 `FiraCode-Regular`。这不是仅检查 CSS 声明；它验证了本机本次加载的实际字体，但不保证所有网络和设备环境下不会发生回退。
